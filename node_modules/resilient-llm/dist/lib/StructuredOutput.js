 ...
 */
export function normalizeStructuredOutputConfig(responseFormat, outputConfigMigration) {
    ...
    if (responseFormat && typeof responseFormat === 'object' && !Array.isArray(responseFormat)) {
        return normalizeFromObjectFormat(responseFormat);
    }
    return { ok: true, data: CONFIG_DEFAULTS };
}
function normalizeFromStringAlias(alias) {
...
function normalizeFromObjectFormat(format) {
...
function normalizeFromOutputConfig(outputConfig) {
 ...
 */
export function mapConfigToRequestFields(config, requestField) {
    if (config._source === 'none')
        return {};
...
function deriveOutputConfig(config) {
...
function deriveResponseFormat(config) {
    if (config.expectsJson && !config.schema) {
 ...
 */
export function parseStructuredResponse(envelope, config) {
 ...
 */
export function validateStructuredResponse(parsedContent, config) {
...
// HELPERS
// ═══════════════════════════════════════════════════════════════════════════
function extractJsonSchema(format) {
...
function extractSchemaName(format) {
    if (format.json_schema && typeof format.json_schema === 'object') {
        const wrapper = format.json_schema;
        if (typeof wrapper.name === 'string')
            return wrapper.name;
    }
    return null;
}
function buildJsonParseCandidates(content) {
    const candidates = [];
...
function validateAgainstSchema(data, schema) {
    ...
    for (const [field, fieldSchema] of Object.entries(properties)) {
        if (!(field in data) || !fieldSchema || typeof fieldSchema !== 'object')
            continue;
...
function schemaPrimitiveType(fieldSchema) {
...
function runtimePrimitiveType(value) {
